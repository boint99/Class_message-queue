...
const receiveMsg = async (exchange) => {
  ...
  const q = await channel.assertQueue(exchange, { durable: true });
  // Liên kết queue với exchange bằng routing key (ở đây sử dụng tên queue làm routing key)
  const bind = await channel.bindQueue(q.queue, exchange, "");
...
// Gọi hàm receiveMsg để nhận message
receiveMsg("system_broadcast");
